/******************************************************************************************************************************
-- Este procedimiento almacenado usp_regusuario_lst
--     es el encargado de realizar consultas a la tabla regusuario
--     , mediante opciones de consultas se manejan los diferentes SELECT a regusuario
--
...
DELIMITER $$
DROP PROCEDURE IF EXISTS sisaguasaniamiento.usp_regusuario_lst $$
CREATE PROCEDURE sisaguasaniamiento.usp_regusuario_lst(
 IN iOpSp int,
...
-- Clases DANT Utilizando  : regusuarioDANT
...
IF iOpSp = 1
    THEN
        SELECT iIdRegUsuario, cCodRegTrabajador, dtFechaVigencia, nvNombUsuario, nvContrasenia, nvObservacion, iIdRegUsuaRegistra, dtFechRegistra, iCantidadModifica, bEstado
        FROM sisaguasaniamiento.regusuario 
        WHERE bEstado=1 AND iIdRegUsuario = _iIdRegUsuario;
    END IF;
...
-- Retorna                 : todas las columnas
-- Parametros Where        : iIdRegUsuario
-- Creado por              : App
-- Fecha creacion          : 22/05/2015 12:09:34
-- ¿Esta siendo Utilizado? : NO
-- Clases DANT Utilizando  : regusuarioDANT
...
-- Retorna                 : iIdRegUsuario y Observacion
-- Parametros Where        : ninguno
-- Creado por              : APP
-- Fecha creacion          : 22/05/2015 12:09:34
-- ¿Esta siendo Utilizado? : NO
...
IF iOpSp = 3
    THEN
        SELECT iIdRegUsuario, nvObservacion
        FROM sisaguasaniamiento.regusuario 
        WHERE bEstado=1;
    END IF;
...
-- Retorna                 : iIdRegUsuario, Observacion y Observacion
-- Parametros Where        : Observacion
-- Creado por              : APP
-- Fecha creacion          : 22/05/2015 12:09:34
-- ¿Esta siendo Utilizado? : NO
...
IF iOpSp = 5
    THEN
        SELECT iIdRegUsuario, nvObservacion, iIdRegUsuaRegistra
        FROM sisaguasaniamiento.regusuario
        WHERE bEstado = 1 AND Observacion LIKE CONCAT("%",_Observacion ,"%")  LIMIT 10; 
    END IF;
...
IF iOpSp = 6
    THEN
    SET @rownum:=0;
	SELECT /*(@rownum:=@rownum+1) as RowNum,*/regusuario.*
	FROM sisaguasaniamiento.regusuario 
	WHERE bEstado=1;
    END IF;
