/******************************************************************************************************************************
-- Este procedimiento almacenado usp_maecategoria_lst
--     es el encargado de realizar consultas a la tabla maecategoria
--     , mediante opciones de consultas se manejan los diferentes SELECT a maecategoria
--
...
DELIMITER $$
DROP PROCEDURE IF EXISTS sisaguasaniamiento.usp_maecategoria_lst $$
CREATE PROCEDURE sisaguasaniamiento.usp_maecategoria_lst(
 IN iOpSp int,
...
-- Clases DANT Utilizando  : maecategoriaDANT
...
IF iOpSp = 1
    THEN
        SELECT cCodMaeCategoria, nvAbreviatura, nvDenominacion, nvDescripcion, nvObservacion, iIdRegUsuaRegistra, dtFechRegistra, iCantidadModifica, bEstado
        FROM sisaguasaniamiento.maecategoria 
        WHERE bEstado=1 AND cCodMaeCategoria = _cCodMaeCategoria;
    END IF;
...
-- Retorna                 : todas las columnas
-- Parametros Where        : cCodMaeCategoria
-- Creado por              : App
-- Fecha creacion          : 22/05/2015 12:09:34
-- ¿Esta siendo Utilizado? : NO
-- Clases DANT Utilizando  : maecategoriaDANT
...
-- Retorna                 : cCodMaeCategoria y Observacion
-- Parametros Where        : ninguno
-- Creado por              : APP
-- Fecha creacion          : 22/05/2015 12:09:34
-- ¿Esta siendo Utilizado? : NO
...
IF iOpSp = 3
    THEN
        SELECT cCodMaeCategoria, nvObservacion
        FROM sisaguasaniamiento.maecategoria 
        WHERE bEstado=1;
    END IF;
...
-- Retorna                 : cCodMaeCategoria, Observacion y Observacion
-- Parametros Where        : Observacion
-- Creado por              : APP
-- Fecha creacion          : 22/05/2015 12:09:34
-- ¿Esta siendo Utilizado? : NO
...
IF iOpSp = 5
    THEN
        SELECT cCodMaeCategoria, nvObservacion, iIdRegUsuaRegistra
        FROM sisaguasaniamiento.maecategoria
        WHERE bEstado = 1 AND Observacion LIKE CONCAT("%",_Observacion ,"%")  LIMIT 10; 
    END IF;
...
IF iOpSp = 6
    THEN
    SET @rownum:=0;
	SELECT /*(@rownum:=@rownum+1) as RowNum,*/maecategoria.*
	FROM sisaguasaniamiento.maecategoria 
	WHERE bEstado=1;
    END IF;
