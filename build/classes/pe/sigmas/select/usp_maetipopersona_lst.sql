/******************************************************************************************************************************
-- Este procedimiento almacenado usp_maetipopersona_lst
--     es el encargado de realizar consultas a la tabla maetipopersona
--     , mediante opciones de consultas se manejan los diferentes SELECT a maetipopersona
--
...
DELIMITER $$
DROP PROCEDURE IF EXISTS sisaguasaniamiento.usp_maetipopersona_lst $$
CREATE PROCEDURE sisaguasaniamiento.usp_maetipopersona_lst(
 IN iOpSp int,
...
-- Clases DANT Utilizando  : maetipopersonaDANT
...
IF iOpSp = 1
    THEN
        SELECT cCodMaeTipoPersona, nvAbreviatura, nvDenominacion, nvDescripcion, nvObservacion, iIdRegUsuaRegistra, dtFechRegistra, iCantidadModifica, bEstado
        FROM sisaguasaniamiento.maetipopersona 
        WHERE bEstado=1 AND cCodMaeTipoPersona = _cCodMaeTipoPersona;
    END IF;
...
-- Retorna                 : todas las columnas
-- Parametros Where        : cCodMaeTipoPersona
-- Creado por              : App
-- Fecha creacion          : 22/05/2015 12:09:34
-- ¿Esta siendo Utilizado? : NO
-- Clases DANT Utilizando  : maetipopersonaDANT
...
-- Retorna                 : cCodMaeTipoPersona y Observacion
-- Parametros Where        : ninguno
-- Creado por              : APP
-- Fecha creacion          : 22/05/2015 12:09:34
-- ¿Esta siendo Utilizado? : NO
...
IF iOpSp = 3
    THEN
        SELECT cCodMaeTipoPersona, nvObservacion
        FROM sisaguasaniamiento.maetipopersona 
        WHERE bEstado=1;
    END IF;
...
-- Retorna                 : cCodMaeTipoPersona, Observacion y Observacion
-- Parametros Where        : Observacion
-- Creado por              : APP
-- Fecha creacion          : 22/05/2015 12:09:34
-- ¿Esta siendo Utilizado? : NO
...
IF iOpSp = 5
    THEN
        SELECT cCodMaeTipoPersona, nvObservacion, iIdRegUsuaRegistra
        FROM sisaguasaniamiento.maetipopersona
        WHERE bEstado = 1 AND Observacion LIKE CONCAT("%",_Observacion ,"%")  LIMIT 10; 
    END IF;
...
IF iOpSp = 6
    THEN
    SET @rownum:=0;
	SELECT /*(@rownum:=@rownum+1) as RowNum,*/maetipopersona.*
	FROM sisaguasaniamiento.maetipopersona 
	WHERE bEstado=1;
    END IF;
