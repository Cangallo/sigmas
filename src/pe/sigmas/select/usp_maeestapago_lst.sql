/******************************************************************************************************************************
-- Este procedimiento almacenado usp_maeestapago_lst
--     es el encargado de realizar consultas a la tabla maeestapago
--     , mediante opciones de consultas se manejan los diferentes SELECT a maeestapago
--
...
DELIMITER $$
DROP PROCEDURE IF EXISTS sisaguasaniamiento.usp_maeestapago_lst $$
CREATE PROCEDURE sisaguasaniamiento.usp_maeestapago_lst(
 IN iOpSp int,
...
-- Clases DANT Utilizando  : maeestapagoDANT
...
IF iOpSp = 1
    THEN
        SELECT cCodMaeEstaPago, nvAbreviatura, nvDenominacion, nvDescripcion, nvObservacion, iIdRegUsuaRegistra, dtFechRegistra, iCantidadModifica, bEstado
        FROM sisaguasaniamiento.maeestapago 
        WHERE bEstado=1 AND cCodMaeEstaPago = _cCodMaeEstaPago;
    END IF;
...
-- Retorna                 : todas las columnas
-- Parametros Where        : cCodMaeEstaPago
-- Creado por              : App
-- Fecha creacion          : 22/05/2015 12:09:34
-- ¿Esta siendo Utilizado? : NO
-- Clases DANT Utilizando  : maeestapagoDANT
...
-- Retorna                 : cCodMaeEstaPago y Observacion
-- Parametros Where        : ninguno
-- Creado por              : APP
-- Fecha creacion          : 22/05/2015 12:09:34
-- ¿Esta siendo Utilizado? : NO
...
IF iOpSp = 3
    THEN
        SELECT cCodMaeEstaPago, nvObservacion
        FROM sisaguasaniamiento.maeestapago 
        WHERE bEstado=1;
    END IF;
...
-- Retorna                 : cCodMaeEstaPago, Observacion y Observacion
-- Parametros Where        : Observacion
-- Creado por              : APP
-- Fecha creacion          : 22/05/2015 12:09:34
-- ¿Esta siendo Utilizado? : NO
...
IF iOpSp = 5
    THEN
        SELECT cCodMaeEstaPago, nvObservacion, iIdRegUsuaRegistra
        FROM sisaguasaniamiento.maeestapago
        WHERE bEstado = 1 AND Observacion LIKE CONCAT("%",_Observacion ,"%")  LIMIT 10; 
    END IF;
...
IF iOpSp = 6
    THEN
    SET @rownum:=0;
	SELECT /*(@rownum:=@rownum+1) as RowNum,*/maeestapago.*
	FROM sisaguasaniamiento.maeestapago 
	WHERE bEstado=1;
    END IF;
