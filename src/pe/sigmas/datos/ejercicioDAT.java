/*******************************************************************************
 * Descripcion       : Clase que contiene los metodos transaccionales de acceso a datos de ejercicio
 * Creado por        : APP
 * Fecha de Creacion : 22/05/2015 12:08:40
** *****************************************************************************/

package pe.sigmas.datos;

import pe.sigmas.entity.ejercicioBean;
import pe.sigmas.conexion.DBConnection;
import java.sql.Connection;
import java.sql.CallableStatement;
import java.sql.ResultSet;
import java.sql.SQLException;
import java.util.ArrayList;
import java.util.LinkedList;
import java.util.List;


/**
*
* @author APP
 */
    public class ejercicioDAT
    {
/* ***************************************************************************************************
 * --
 * -- Creado por              : APP
 * -- Fecha creacion          : 22/05/2015 12:08:40
 * -- ¿Esta siendo Utilizado? : NO
 * -- Modificado por          : <Analista Programador>
 * -- Fecha Actualizacion     : <Fecha de Actualizacion>
 * --
 * **************************************************************************************************/
  /**
  * @fnIsejercicioBean, es un metodo que retorna si existe un ejercicioBean metodo efectivo para validaciones
  * @param poejercicioBean, Objeto correspondiente a ejercicioBean
  * @return Retorna true cuando existe un ejercicioBean, false cuando no existe un ejercicioBean
  * @throws Exception
  */
        public boolean fnIsejercicioBean (ejercicioBean poejercicioBean) throws Exception 
        {
        ejercicioBean oejercicioBean = null;
        Connection cn = null ;
        ResultSet rs = null;
        CallableStatement cs = null;
        try {
        cn = DBConnection.getInstance().openConnection();
        cn.setAutoCommit(false);
        cs = cn.prepareCall("{CALL sisaguasaniamiento.usp_ejercicio_lst(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ? )}");
        cs.setInt(1, poejercicioBean.getIOps());
        cs.registerOutParameter(2, java.sql.Types.BOOLEAN);
        cs.setInt(3, poejercicioBean.getIIdEjercicio());
        cs.setInt(4, poejercicioBean.getIAnioFiscal());
        cs.setDouble(5, poejercicioBean.getDeUit());
        cs.setString(6, poejercicioBean.getNvDenominacion());
        cs.setString(7, poejercicioBean.getNvObservacion());
        cs.setInt(8, poejercicioBean.getIIdRegUsuaRegistra());
        cs.setDate(9, poejercicioBean.getDtFechRegistra());
        cs.setInt(10, poejercicioBean.getICantidadModifica());
        cs.setBoolean(11, poejercicioBean.getBEstado());
        rs = cs.executeQuery();
        while (rs.next()) {
        oejercicioBean = new ejercicioBean();
        oejercicioBean.setBEstado(rs.getBoolean("iIdEjercicio"));
        }
        cn.commit();
        cn.setAutoCommit(true);
        } catch (SQLException ex) {
        cn.rollback();
        ex.printStackTrace();
        } finally {
        rs.close();
        rs = null;
        cn.close();
        cn = null;
        }
        return oejercicioBean!= null;
        }
/* ***************************************************************************************************
 * --
 * -- Creado por              : APP
 * -- Fecha creacion          : 22/05/2015 12:08:40
 * -- ¿Esta siendo Utilizado? : NO
 * -- Modificado por          : <Analista Programador>
 * -- Fecha Actualizacion     : <Fecha de Actualizacion>
 * --
 * **************************************************************************************************/
  /**
  * @fnGetListejercicioBean, es un metodo que retorna una lista de objetos de ejercicioBean metodo efectivo para cargar controles de seleccion y/o controles de aucompletado
  * @param poejercicioBean, Objeto correspondiente a ejercicioBean
  * @return Retorna una lista de objetos ejercicioBean
  * @throws Exception
  */
  public List fnGetListejercicioBean(ejercicioBean poejercicioBean) throws Exception {
  List list = new LinkedList();
  Connection cn = null;
  ResultSet rs = null;
        CallableStatement cs = null;
        try {
        cn = DBConnection.getInstance().openConnection();
        cn.setAutoCommit(false);
        cs = cn.prepareCall("{CALL sisaguasaniamiento.usp_ejercicio_lst(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ? )}");
        cs.setInt(1, poejercicioBean.getIOps());
        cs.registerOutParameter(2, java.sql.Types.BOOLEAN);
        cs.setInt(3, poejercicioBean.getIIdEjercicio());
        cs.setInt(4, poejercicioBean.getIAnioFiscal());
        cs.setDouble(5, poejercicioBean.getDeUit());
        cs.setString(6, poejercicioBean.getNvDenominacion());
        cs.setString(7, poejercicioBean.getNvObservacion());
        cs.setInt(8, poejercicioBean.getIIdRegUsuaRegistra());
        cs.setDate(9, poejercicioBean.getDtFechRegistra());
        cs.setInt(10, poejercicioBean.getICantidadModifica());
        cs.setBoolean(11, poejercicioBean.getBEstado());
        rs = cs.executeQuery();
  while (rs.next()) {
  Object[] obj = {rs.getString(1), rs.getString(2), rs.getString(3), rs.getString(4), rs.getString(5), rs.getString(6), rs.getString(7), rs.getString(8), rs.getString(9)}; 
  //Object[] obj = {rs.getInt(1), rs.getInt(2), rs.getDouble(3), rs.getString(4), rs.getString(5), rs.getInt(6), rs.getDate(7), rs.getInt(8), rs.getBoolean(9)}; 
  list.add(obj); 
  }
  cn.commit();
  cn.setAutoCommit(true);
  } catch (SQLException ex) {
  cn.rollback();
  ex.printStackTrace();
  } finally {
  rs.close();
  rs = null;
  cn.close();
  cn = null;
  }
  return list;
  }
/* ***************************************************************************************************
 * --
 * -- Creado por              : APP
 * -- Fecha creacion          : 22/05/2015 12:08:40
 * -- ¿Esta siendo Utilizado? : NO
 * -- Modificado por          : <Analista Programador>
 * -- Fecha Actualizacion     : <Fecha de Actualizacion>
 * --
 * **************************************************************************************************/
  /**
  * @fnGridejercicioBean, es un metodo que retorna una lista de objetos de ejercicioBean metodo efectivo para cargar controles de seleccion y/o controles de aucompletado
  * @param poejercicioBean, Objeto correspondiente a ejercicioBean
  * @return Retorna un jQgrid efectivo para cargar y mostrar registros en una grilla
  * @throws Exception
  */
  public List fnGridejercicioBean(ejercicioBean poejercicioBean) throws Exception {
  List lista= new LinkedList();
  Connection cn = null;
  ResultSet rs = null;
        CallableStatement cs = null;
        try {
        cn = DBConnection.getInstance().openConnection();
        cn.setAutoCommit(false);
        cs = cn.prepareCall("{CALL sisaguasaniamiento.usp_ejercicio_lst(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ? )}");
        cs.setInt(1, poejercicioBean.getIOps());
        cs.registerOutParameter(2, java.sql.Types.BOOLEAN);
        cs.setInt(3, poejercicioBean.getIIdEjercicio());
        cs.setInt(4, poejercicioBean.getIAnioFiscal());
        cs.setDouble(5, poejercicioBean.getDeUit());
        cs.setString(6, poejercicioBean.getNvDenominacion());
        cs.setString(7, poejercicioBean.getNvObservacion());
        cs.setInt(8, poejercicioBean.getIIdRegUsuaRegistra());
        cs.setDate(9, poejercicioBean.getDtFechRegistra());
        cs.setInt(10, poejercicioBean.getICantidadModifica());
        cs.setBoolean(11, poejercicioBean.getBEstado());
        rs = cs.executeQuery();
  while (rs.next()) {
  Object[] obj = {rs.getString(1), rs.getString(2), rs.getString(3), rs.getString(4), rs.getString(5), rs.getString(6), rs.getString(7), rs.getString(8), rs.getString(9)}; 
  //Object[] obj = {rs.getInt(1), rs.getInt(2), rs.getDouble(3), rs.getString(4), rs.getString(5), rs.getInt(6), rs.getDate(7), rs.getInt(8), rs.getBoolean(9)}; 
  lista.add(obj); 
  }
  cn.commit();
  cn.setAutoCommit(true);
  } catch (SQLException ex) {
  cn.rollback();
  ex.printStackTrace();
  } finally {
  rs.close();
  rs = null;
  cn.close();
  cn = null;
  }
  return lista;
  }
/* ***************************************************************************************************
 * --
 * -- Creado por              : APP
 * -- Fecha creacion          : 22/05/2015 12:08:40
 * -- ¿Esta siendo Utilizado? : NO
 * -- Modificado por          : <Analista Programador>
 * -- Fecha Actualizacion     : <Fecha de Actualizacion>
 * --
 * **************************************************************************************************/
  /**
  * @fnAutocompleteejercicioBean, es un metodo que retorna una lista de objetos de ejercicioBean metodo efectivo para cargar controles de seleccion y/o controles de aucompletado
  * @param poejercicioBean, Objeto correspondiente a ejercicioBean
  * @return Retorna una lista efectivo para cargar y mostrar registros en un autocomplete
  * @throws Exception
  */
  public ArrayList<ejercicioBean> fnAutocompleteejercicioBean(ejercicioBean poejercicioBean) throws Exception {
  ArrayList<ejercicioBean> listaejercicioBean = new ArrayList<>();
  Connection cn = null;
  ResultSet rs = null;
        CallableStatement cs = null;
        try {
        cn = DBConnection.getInstance().openConnection();
        cn.setAutoCommit(false);
        cs = cn.prepareCall("{CALL sisaguasaniamiento.usp_ejercicio_lst(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ? )}");
        cs.setInt(1, poejercicioBean.getIOps());
        cs.registerOutParameter(2, java.sql.Types.BOOLEAN);
        cs.setInt(3, poejercicioBean.getIIdEjercicio());
        cs.setInt(4, poejercicioBean.getIAnioFiscal());
        cs.setDouble(5, poejercicioBean.getDeUit());
        cs.setString(6, poejercicioBean.getNvDenominacion());
        cs.setString(7, poejercicioBean.getNvObservacion());
        cs.setInt(8, poejercicioBean.getIIdRegUsuaRegistra());
        cs.setDate(9, poejercicioBean.getDtFechRegistra());
        cs.setInt(10, poejercicioBean.getICantidadModifica());
        cs.setBoolean(11, poejercicioBean.getBEstado());
        rs = cs.executeQuery();
  while (rs.next()) {
  ejercicioBean oejercicioBean = new ejercicioBean();
  oejercicioBean.setIIdEjercicio(rs.getInt("iIdEjercicio")); 
  oejercicioBean.setIAnioFiscal(rs.getInt("iAnioFiscal")); 
  oejercicioBean.setDeUit(rs.getDouble("deUit")); 
  oejercicioBean.setNvDenominacion(rs.getString("nvDenominacion")); 
  oejercicioBean.setNvObservacion(rs.getString("nvObservacion")); 
  oejercicioBean.setIIdRegUsuaRegistra(rs.getInt("iIdRegUsuaRegistra")); 
  oejercicioBean.setDtFechRegistra(rs.getDate("dtFechRegistra")); 
  oejercicioBean.setICantidadModifica(rs.getInt("iCantidadModifica")); 
  oejercicioBean.setBEstado(rs.getBoolean("bEstado")); 
  listaejercicioBean.add(oejercicioBean);
  }
  cn.commit();
  cn.setAutoCommit(true);
  } catch (SQLException ex) {
  cn.rollback();
  ex.printStackTrace();
  } finally {
  rs.close();
  rs = null;
  cn.close();
  cn = null;
  }
  return listaejercicioBean;
  }
/* ***************************************************************************************************
 * --
 * -- Creado por              : APP
 * -- Fecha creacion          : 22/05/2015 12:08:40
 * -- ¿Esta siendo Utilizado? : NO
 * -- Modificado por          : <Analista Programador>
 * -- Fecha Actualizacion     : <Fecha de Actualizacion>
 * --
 * **************************************************************************************************/
  /**
  * @fnGetejercicioBean, es un metodo que retorna un objeto especifico de ejercicioBean
  * @param poejercicioBean, Objeto correspondiente a ejercicioBean
  * @return Retorna el Objeto ejercicioBean
  * @throws Exception
  */
  public ejercicioBean fnGetejercicioBean(ejercicioBean poejercicioBean) throws Exception {
  ejercicioBean oejercicioBean = null;
  Connection cn = null;
  ResultSet rs = null;
        CallableStatement cs = null;
        try {
        cn = DBConnection.getInstance().openConnection();
        cn.setAutoCommit(false);
        cs = cn.prepareCall("{CALL sisaguasaniamiento.usp_ejercicio_lst(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ? )}");
        cs.setInt(1, poejercicioBean.getIOps());
        cs.registerOutParameter(2, java.sql.Types.BOOLEAN);
        cs.setInt(3, poejercicioBean.getIIdEjercicio());
        cs.setInt(4, poejercicioBean.getIAnioFiscal());
        cs.setDouble(5, poejercicioBean.getDeUit());
        cs.setString(6, poejercicioBean.getNvDenominacion());
        cs.setString(7, poejercicioBean.getNvObservacion());
        cs.setInt(8, poejercicioBean.getIIdRegUsuaRegistra());
        cs.setDate(9, poejercicioBean.getDtFechRegistra());
        cs.setInt(10, poejercicioBean.getICantidadModifica());
        cs.setBoolean(11, poejercicioBean.getBEstado());
        rs = cs.executeQuery();
   while (rs.next()) {
  oejercicioBean = new ejercicioBean();
  oejercicioBean.setIIdEjercicio(rs.getInt("iIdEjercicio")); 
  oejercicioBean.setIAnioFiscal(rs.getInt("iAnioFiscal")); 
  oejercicioBean.setDeUit(rs.getDouble("deUit")); 
  oejercicioBean.setNvDenominacion(rs.getString("nvDenominacion")); 
  oejercicioBean.setNvObservacion(rs.getString("nvObservacion")); 
  oejercicioBean.setIIdRegUsuaRegistra(rs.getInt("iIdRegUsuaRegistra")); 
  oejercicioBean.setDtFechRegistra(rs.getDate("dtFechRegistra")); 
  oejercicioBean.setICantidadModifica(rs.getInt("iCantidadModifica")); 
  oejercicioBean.setBEstado(rs.getBoolean("bEstado")); 
  }
  cn.commit();
  cn.setAutoCommit(true);
  } catch (SQLException ex) {
  cn.rollback();
  ex.printStackTrace();
  } finally {
  rs.close();
  rs = null;
  cn.close();
  cn = null;
  }
  return oejercicioBean;
  }
/* ***************************************************************************************************
 * --
 * -- Creado por              : APP
 * -- Fecha creacion          : 22/05/2015 12:08:40
 * -- ¿Esta siendo Utilizado? : NO
 * -- Modificado por          : <Analista Programador>
 * -- Fecha Actualizacion     : <Fecha de Actualizacion>
 * --
 * **************************************************************************************************/
  /**
  * @fnInsertarejercicioBean,  es un metodo que Inserta un  ejercicioBean
  * @param poejercicioBean, Objeto correspondiente a ejercicioBean
  * @return Retorna true cuando es exito, false cuando no hubo insercion alguna
  * @throws Exception
  */
  public boolean fnInsertarejercicioBean(ejercicioBean poejercicioBean) throws Exception {
  boolean resultado = false;
  Connection cn = null;
        CallableStatement cs = null;
        try {
        cn = DBConnection.getInstance().openConnection();
        cn.setAutoCommit(false);
        cs = cn.prepareCall("{CALL sisaguasaniamiento.usp_ejercicio_ins(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ? )}");
        cs.setInt(1, poejercicioBean.getIOps());
        cs.registerOutParameter(2, java.sql.Types.BOOLEAN);
        cs.setInt(3, poejercicioBean.getIIdEjercicio());
        cs.setInt(4, poejercicioBean.getIAnioFiscal());
        cs.setDouble(5, poejercicioBean.getDeUit());
        cs.setString(6, poejercicioBean.getNvDenominacion());
        cs.setString(7, poejercicioBean.getNvObservacion());
        cs.setInt(8, poejercicioBean.getIIdRegUsuaRegistra());
        cs.setDate(9, poejercicioBean.getDtFechRegistra());
        cs.setInt(10, poejercicioBean.getICantidadModifica());
        cs.setBoolean(11, true);
         cs.execute();
        resultado = cs.getBoolean(2);
  cs.close();
  cs = null;
  cn.commit();
  cn.setAutoCommit(true);
  } catch (SQLException ex) {
  cn.rollback();
  ex.printStackTrace();
  resultado = false;
  } finally {
   cn.close();
  cn = null;
  }
   return resultado;
  }
/* ***************************************************************************************************
 * --
 * -- Creado por              : APP
 * -- Fecha creacion          : 22/05/2015 12:08:40
 * -- ¿Esta siendo Utilizado? : NO
 * -- Modificado por          : <Analista Programador>
 * -- Fecha Actualizacion     : <Fecha de Actualizacion>
 * --
 * **************************************************************************************************/
  /**
  * @fnEliminarejercicioBean,  es un metodo que Elimina un  ejercicioBean
  * @param poejercicioBean, Objeto correspondiente a ejercicioBean
  * @return Retorna true cuando es exito, false cuando no hubo eliminacion alguna
  * @throws Exception
  */
  public boolean fnEliminarejercicioBean(ejercicioBean poejercicioBean) throws Exception {
  boolean resultado = false;
  Connection cn = null;
        CallableStatement cs = null;
        try {
        cn = DBConnection.getInstance().openConnection();
        cn.setAutoCommit(false);
        cs = cn.prepareCall("{CALL sisaguasaniamiento.usp_ejercicio_upd(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ? )}");
        cs.setInt(1, poejercicioBean.getIOps());
        cs.registerOutParameter(2, java.sql.Types.BOOLEAN);
        cs.setInt(3, poejercicioBean.getIIdEjercicio());
        cs.setInt(4, poejercicioBean.getIAnioFiscal());
        cs.setDouble(5, poejercicioBean.getDeUit());
        cs.setString(6, poejercicioBean.getNvDenominacion());
        cs.setString(7, poejercicioBean.getNvObservacion());
        cs.setInt(8, poejercicioBean.getIIdRegUsuaRegistra());
        cs.setDate(9, poejercicioBean.getDtFechRegistra());
        cs.setInt(10, poejercicioBean.getICantidadModifica());
        cs.setBoolean(11, false);
         cs.execute();
        resultado = cs.getBoolean(2);
  cs.close();
  cs = null;
  cn.commit();
  cn.setAutoCommit(true);
  } catch (SQLException ex) {
  cn.rollback();
  ex.printStackTrace();
  resultado = false;
  } finally {
  cn.close();
  cn = null;
  }
   return resultado;
  }
/* ***************************************************************************************************
 * --
 * -- Creado por              : APP
 * -- Fecha creacion          : 22/05/2015 12:08:40
 * -- ¿Esta siendo Utilizado? : NO
 * -- Modificado por          : <Analista Programador>
 * -- Fecha Actualizacion     : <Fecha de Actualizacion>
 * --
 * **************************************************************************************************/
  /**
  * @fnActualizarejercicioBean,  es un metodo que Actualiza un  ejercicioBean
  * @param poejercicioBean, Objeto correspondiente a ejercicioBean
  * @return Retorna true cuando es exito, false cuando no hubo actualizacion alguna
  * @throws Exception
  */
  public boolean fnActualizarejercicioBean(ejercicioBean poejercicioBean) throws Exception {
  boolean resultado = false;
  Connection cn = null;
        CallableStatement cs = null;
        try {
        cn = DBConnection.getInstance().openConnection();
        cn.setAutoCommit(false);
        cs = cn.prepareCall("{CALL sisaguasaniamiento.usp_ejercicio_upd(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ? )}");
        cs.setInt(1, poejercicioBean.getIOps());
        cs.registerOutParameter(2, java.sql.Types.BOOLEAN);
        cs.setInt(3, poejercicioBean.getIIdEjercicio());
        cs.setInt(4, poejercicioBean.getIAnioFiscal());
        cs.setDouble(5, poejercicioBean.getDeUit());
        cs.setString(6, poejercicioBean.getNvDenominacion());
        cs.setString(7, poejercicioBean.getNvObservacion());
        cs.setInt(8, poejercicioBean.getIIdRegUsuaRegistra());
        cs.setDate(9, poejercicioBean.getDtFechRegistra());
        cs.setInt(10, poejercicioBean.getICantidadModifica());
        cs.setBoolean(11, true);
         cs.execute();
        resultado = cs.getBoolean(2);
  cs.close();
  cs = null;
  cn.commit();
  cn.setAutoCommit(true);
  resultado = true;
  } catch (SQLException ex) {
   cn.rollback();
  ex.printStackTrace();
  resultado = false;
  } finally {
  cn.close();
  cn = null;
  } 
  return resultado;
  }
}
